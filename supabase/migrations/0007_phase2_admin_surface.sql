...
drop policy if exists "pending: host org admin read" on pending_registrations;
create policy "pending: host org admin read"
  on pending_registrations for select
  using (is_org_admin(host_org_id));

drop policy if exists "pending: host org admin write" on pending_registrations;
create policy "pending: host org admin write"
  on pending_registrations for insert
  with check (is_org_admin(host_org_id));

drop policy if exists "pending: host org admin update" on pending_registrations;
create policy "pending: host org admin update"
  on pending_registrations for update
  using (is_org_admin(host_org_id));

drop policy if exists "pending: host org admin delete" on pending_registrations;
...
drop policy if exists "webhook_events: host org admin read" on webhook_events;
...
drop policy if exists "roster: member reads own rows" on org_roster_visibility;
create policy "roster: member reads own rows"
  on org_roster_visibility for select
  using (auth.uid() = member_id);

drop policy if exists "roster: member writes own rows" on org_roster_visibility;
create policy "roster: member writes own rows"
  on org_roster_visibility for insert
  with check (auth.uid() = member_id);

drop policy if exists "roster: member updates own rows" on org_roster_visibility;
create policy "roster: member updates own rows"
  on org_roster_visibility for update
  using (auth.uid() = member_id);

drop policy if exists "roster: member deletes own rows" on org_roster_visibility;
...
drop policy if exists "roster: org admin reads all for their org" on org_roster_visibility;
...
drop policy if exists "item_regs: host org admin insert" on item_registrations;
