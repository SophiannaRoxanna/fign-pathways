import Link from "next/link";
import { getSupabaseServer } from "@/lib/supabase/server";
import { C } from "@/lib/design/tokens";
import { SectionHead } from "@/components/ui/SectionHead";
import { OrgChip } from "@/components/org/OrgChip";
import type { Item, Organisation } from "@/lib/supabase/types";

export default async function AdminItemsPage() {
  const supabase = await getSupabaseServer();
  const [{ data: itemsData }, { data: orgsData }] = await Promise.all([
    supabase
      .from("items")
      .select("*")
      .order("posted_at", { ascending: false })
      .limit(500),
    supabase.from("organisations").select("*").limit(500),
  ]);

  const items = (itemsData ?? []) as Item[];
  const orgs = (orgsData ?? []) as Organisation[];
  const orgById = new Map(orgs.map((o) => [o.id, o]));

  return (
    <div>
      <div className="flex items-end justify-between gap-4 mb-7">
        <SectionHead num="02" kicker="the feed">
          Items across the network.
        </SectionHead>
        <Link
          href="/admin/items/new"
          className="font-mono text-[11px] tracking-[0.18em] uppercase font-bold px-5 py-3"
          style={{
            background: C.ink,
            color: C.paper,
            border: `1.5px solid ${C.ink}`,
          }}
        >
          + new item
        </Link>
      </div>

      <div style={{ border: `1.5px solid ${C.ink}` }}>
        <div
          className="grid grid-cols-[auto_auto_auto_1fr_auto_auto_auto] gap-4 px-5 py-3 font-mono text-[10px] tracking-[0.2em] uppercase items-center"
          style={{
            background: C.paperAlt,
            color: C.inkMute,
            borderBottom: `1.5px solid ${C.ink}`,
          }}
        >
          <span>host</span>
          <span>co-hosts</span>
          <span>kind</span>
          <span>title</span>
          <span>when</span>
          <span>vis</span>
          <span>edit</span>
        </div>
        {items.length === 0 ? (
          <div
            className="px-5 py-8 text-center italic font-display text-lg"
            style={{ color: C.inkMute }}
          >
            No items posted. <Link href="/admin/items/new" style={{ color: C.coral }}>Post the first one →</Link>
          </div>
        ) : (
          items.map((it, i) => {
            const host = orgById.get(it.host_org_id);
            const coHosts = it.co_host_org_ids
              .map((id) => orgById.get(id))
              .filter(Boolean) as Organisation[];
            return (
              <div
                key={it.id}
                className="grid grid-cols-[auto_auto_auto_1fr_auto_auto_auto] gap-4 px-5 py-3 items-center"
                style={{
                  background: i % 2 ? C.paper : C.paperAlt,
                  borderBottom:
                    i < items.length - 1 ? `1px solid ${C.ink}22` : undefined,
                }}
              >
                {host ? (
                  <OrgChip org={host} />
                ) : (
                  <span style={{ color: C.inkMute }}>—</span>
                )}
                <div className="flex flex-wrap gap-1">
                  {coHosts.length === 0 ? (
                    <span
                      className="font-mono text-[10px]"
                      style={{ color: C.inkMute }}
                    >
                      —
                    </span>
                  ) : (
                    coHosts.map((co) => <OrgChip key={co.id} org={co} />)
                  )}
                </div>
                <span
                  className="font-mono text-[10px] tracking-[0.18em] uppercase"
                  style={{
                    color: C.paper,
                    background: C.inkSoft,
                    padding: "2px 6px",
                  }}
                >
                  {it.kind}
                </span>
                <span
                  className="font-display text-[17px] truncate"
                  style={{ color: C.ink }}
                >
                  {it.title}
                </span>
                <span
                  className="font-mono text-[10px] tracking-wider uppercase"
                  style={{ color: C.inkSoft }}
                >
                  {it.when_start
                    ? new Date(it.when_start).toLocaleDateString("en", {
                        timeZone: "UTC",
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                      })
                    : it.rolling
                      ? "rolling"
                      : "—"}
                </span>
                <span
                  className="font-mono text-[10px] tracking-[0.18em] uppercase"
                  style={{ color: C.inkMute }}
                >
                  {it.visibility}
                </span>
                <Link
                  href={`/admin/items/${it.id}`}
                  className="font-mono text-[10px] tracking-[0.18em] uppercase font-bold"
                  style={{ color: C.coral }}
                >
                  edit →
                </Link>
              </div>
            );
          })
        )}
      </div>
    </div>
  );
}
